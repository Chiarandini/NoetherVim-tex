...
describe("accent_spell native-spell suppression (extmarks)", function()
  ...
  it("attaches hl_group=SpellBad for typo tokens", function()
    local buf = setup_buf({ "K\\\"ohler manifold." })
    accent.refresh(buf)
    local marks = suppress_extmarks(buf)
    assert.are.equal(1, #marks)
  ...
  it("does NOT attach hl_group for correctly-spelled accented tokens", function()
    -- Add the decoded form first so the token is "correct" by spell.
    vim.cmd("silent! spellgood! Kähler")
    local buf = setup_buf({ "K\\\"ahler form." })
    accent.refresh(buf)
    local marks = suppress_extmarks(buf)
    assert.are.equal(1, #marks)
    assert.is_nil(marks[1][4].hl_group)
    vim.api.nvim_buf_delete(buf, { force = true })
  end)
end)
